...
export class ClaudeCodeHarness implements AgentHarness {
  ...
  constructor(config: ModelConfig) {
    this.executable = config.harnessOpts?.executable ?? 'claude';
  ...
  async run(opts: HarnessRunOpts): Promise<HarnessResult> {
    ...
    const args = ['-p', opts.task, '--output-format', 'json', ...this.extraArgs];

    logger.debug(`[claude-code] spawning: ${this.executable} ${args.join(' ')}`);
    ...
    return new Promise<HarnessResult>((resolve) => {
      ...
      child.on('close', (code) => {
        ...
        if (stderr) logger.debug(`[claude-code] stderr: ${stderr}`);
      ...
      child.on('error', (err) => {
        ...
        logger.warn(`[claude-code] process error: ${err.message}`);
        resolve({
