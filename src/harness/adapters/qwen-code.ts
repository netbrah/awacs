...
export class QwenCodeHarness implements AgentHarness {
  ...
  constructor(config: ModelConfig) {
    this.executable = config.harnessOpts?.executable ?? 'qwen-code';
  ...
  async run(opts: HarnessRunOpts): Promise<HarnessResult> {
    ...
    const args = ['-p', opts.task, '--json', ...this.extraArgs];

    logger.debug(`[qwen-code] spawning: ${this.executable} ${args.join(' ')}`);
    ...
    return new Promise<HarnessResult>((resolve) => {
      ...
      child.on('close', (code) => {
        ...
        if (stderr) logger.debug(`[qwen-code] stderr: ${stderr}`);
        const durationMs = Date.now() - start;

        let content = stdout;
        try {
          const parsed = JSON.parse(stdout) as { result?: string; output?: string; content?: string };
          content = parsed.result ?? parsed.output ?? parsed.content ?? stdout;
        } catch {
      ...
      child.on('error', (err) => {
        ...
        logger.warn(`[qwen-code] process error: ${err.message}`);
        resolve({
